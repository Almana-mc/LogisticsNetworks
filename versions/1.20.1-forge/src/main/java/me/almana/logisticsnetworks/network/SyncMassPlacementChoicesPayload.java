...
public record SyncMassPlacementChoicesPayload(
    ...
    public record BlockChoice(ResourceLocation blockId, String name, int targetCount, boolean selected) {
    }

    public static final CustomPacketPayload.Type<SyncMassPlacementChoicesPayload> TYPE = new CustomPacketPayload.Type<>(
            ResourceLocation.fromNamespaceAndPath(LogisticsNetworks.MOD_ID, "sync_mass_placement_choices"));
    ...
    public static SyncMassPlacementChoicesPayload read(FriendlyByteBuf buf) {
        ...
        for (int i = 0; i < count; i++) {
            ResourceLocation blockId = ResourceLocation.tryParse(buf.readUtf(128));
