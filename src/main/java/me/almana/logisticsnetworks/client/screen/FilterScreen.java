...
public class FilterScreen extends AbstractContainerScreen<FilterMenu> {
    ...
    public boolean mouseScrolled(double mx, double my, double delta) {
        ...
        if (nbtEditSlot >= 0 && nbtInputBox != null) {
            return nbtInputBox.mouseScrolled(mx, my, delta);
        }
    ...
    private boolean handleNbtSubModeClick(double mx, double my, int btn) {
        ...
        int clearY = panelY + 4;
        if (isHovering(clearX, clearY, clearW, 12, (int) mx, (int) my)) {
            NetworkHandler.sendToServer(new SetFilterEntryNbtPayload(nbtEditSlot, "", false));
    ...
    private void commitNbtInput() {
        ...
        if (!val.isEmpty()) {
            NetworkHandler.sendToServer(
                    new SetFilterEntryNbtPayload(nbtEditSlot, "", true, val));
        }
        closeNbtSubMode();
    }
    ...
    private void applyNbtSelection() {
        ...
        if (!valueOverride.isEmpty() && !valueOverride.equals(entry.valueDisplay())) {
            NetworkHandler.sendToServer(new SetFilterEntryNbtPayload(
                    detailEditSlot, entry.path(), true, valueOverride, op.id()));
            menu.setEntryNbtWithValue(minecraft.player, detailEditSlot, entry.path(), valueOverride, op.id());
    ...
    private void commitBuiltinEdit() {
    ...
    private void flushNbtSubPage() {
        ...
        if (detailNbtRawMode) {
            ...
            if (!nbtVal.isEmpty() && !nbtVal.equals(existingRaw)) {
                NetworkHandler.sendToServer(new SetFilterEntryNbtPayload(detailEditSlot, "", true, nbtVal));
    ...
    private void sendDurabilityPacket() {
        if (detailDurabilityOp != null) {
