...
import me.almana.logisticsnetworks.client.graph.GraphRenderState;
...
public final class WrenchModeScreen extends Screen {
    ...
    public void tick() {
        if (!minecraft.isWindowActive() || minecraft.player == null
            ...
            if (selected >= 0) ClientPacketDistributor.sendToServer(new SetWrenchModePayload(hand, WrenchItem.Mode.values()[selected]));
            onClose();
        }
    }
    ...
    private void drawSector(GuiGraphics graphics, int sector, int color) {
        var vertices = new GraphRenderState.Builder();
        float x = width / 2f;
        float y = height / 2f;
        for (int step = 0; step < 30; step++) {
            double a = Math.toRadians(-149 + sector * 120 + step * 118.0 / 30);
            double b = Math.toRadians(-149 + sector * 120 + (step + 1) * 118.0 / 30);
            vertices.vertex(x + (float) Math.cos(a) * INNER_RADIUS, y + (float) Math.sin(a) * INNER_RADIUS, color);
            vertices.vertex(x + (float) Math.cos(b) * INNER_RADIUS, y + (float) Math.sin(b) * INNER_RADIUS, color);
            vertices.vertex(x + (float) Math.cos(b) * OUTER_RADIUS, y + (float) Math.sin(b) * OUTER_RADIUS, color);
            vertices.vertex(x + (float) Math.cos(a) * OUTER_RADIUS, y + (float) Math.sin(a) * OUTER_RADIUS, color);
        }
