...
public final class NodeConnectionMask {
    ...
    public static int remove(int mask, Direction direction) {
        return mask & ~bit(direction);
    }

    public static int addCorner(int mask, Direction first, Direction second) {
        return mask | cornerBit(first, second);
    }

    public static boolean hasCorner(int mask, Direction first, Direction second) {
        return (mask & cornerBit(first, second)) != 0;
    }

    public static boolean isCornerPair(Direction first, Direction second) {
