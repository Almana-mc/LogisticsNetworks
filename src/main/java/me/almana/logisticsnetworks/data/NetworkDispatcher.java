...
final class NetworkDispatcher {
    ...
    static void drainCompleted(Supplier<TransferPlan> pollCompleted,
        ...
        while ((plan = pollCompleted.get()) != null) {
            committed |= commit.test(plan);
            if (committed && nanoTime.getAsLong() - startedAt >= budgetNanos && !hasTime.getAsBoolean()) {
                return;
            }
        }
    }
    ...
    void recordDispatchException(LogisticsNetwork network, long generation, long runtimeId) {
        UUID id = network.getId();
        AsyncTransferRuntime current = AsyncTransferRuntime.get();
        long currentRuntimeId = current == null ? -1L : current.runtimeId();
        ...
        boolean newlyDisabled = state.finishWorkerPlan(new TransferPlan(
                id, generation, runtimeId, true, Long.MAX_VALUE, List.of()),
                network.getGeneration(), currentRuntimeId);
        if (newlyDisabled) {
    ...
    private void commitCurrentPlan(TransferPlan plan, LogisticsNetwork network,
        ...
        try {
            ...
            long synchronousDelta = TransferEngine.processSynchronousNonItems(network, server);
    ...
    private static void warnHighDispatch(int count) {
        if (Config.debugMode && count > WARNING_DISPATCH_COUNT) {
