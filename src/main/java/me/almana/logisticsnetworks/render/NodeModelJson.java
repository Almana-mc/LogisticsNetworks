...
public final class NodeModelJson {

    private NodeModelJson() {
    }
    ...
    private static JsonObject readJson(String resourcePath) {
        InputStream stream = NodeModelJson.class.getResourceAsStream(resourcePath);
        if (stream == null) {
            throw new IllegalStateException("Model not found: " + resourcePath);
        }
    ...
    private static void parseBone(PartDefinition parent, JsonObject bone) {
        ...
        if (bone.has("cubes")) {
            for (JsonElement cubeElement : bone.getAsJsonArray("cubes")) {
                ...
                cubeList.texOffs(u, v).addBox(ox, oy, oz, sx, sy, sz, new CubeDeformation(inflate));
            }
        }

        PartDefinition part = parent.addOrReplaceChild(name, cubeList, parsePose(bone));
    ...
    private static PartPose parsePose(JsonObject bone) {
        ...
        if (bone.has("rotation")) {
            ...
            float rx = (float) Math.toRadians(rotation.get(0).getAsDouble());
            float ry = (float) Math.toRadians(rotation.get(1).getAsDouble());
            float rz = (float) Math.toRadians(rotation.get(2).getAsDouble());
