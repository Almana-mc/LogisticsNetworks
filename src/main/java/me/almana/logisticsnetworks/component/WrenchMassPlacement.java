...
public record WrenchMassPlacement(
        Optional<Area> area,
        Optional<Identifier> selectedBlock,
    ...
    public static final Codec<WrenchMassPlacement> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Identifier.CODEC.optionalFieldOf("selected_block").forGetter(WrenchMassPlacement::selectedBlock),
    ...
    public WrenchMassPlacement {
        ...
        selections = selections.stream().map(pos -> GlobalPos.of(pos.dimension(), pos.pos().immutable())).toList();
    }
    ...
    public record Area(GlobalPos first, Optional<BlockPos> second) {
        ...
        public Area {
            first = GlobalPos.of(first.dimension(), first.pos().immutable());
            second = second == null ? Optional.empty() : second.map(BlockPos::immutable);
        }
    }
}
